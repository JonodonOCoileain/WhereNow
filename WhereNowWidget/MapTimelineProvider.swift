...
public struct MapTimelineEntry: TimelineEntry {
    // MARK: - Types
...
//convert to AppIntentTimeLineProvider
struct MapTimelineProvider: TimelineProvider {
    /*func snapshot(for configuration: ConfigurationAppIntent, in context: Context) async -> SimpleEntry {
        SimpleEntry(date: Date(), addresses: mapSnapshotManager., configuration: configuration)
    }*/
    ...
    init(locationManager: LocationManager, mapSnapshotManager: SnapshotManager) {
            ...
            case let .success(userLocation):
                ...
                mapSnapshotManager.snapshot(at: userLocation.coordinate) { mapSnapshotResult in
                    ...
                    case let .success(mapImage):
                        ...
                        guard let url = URL(string: "https://api.tomtom.com/search/2/reverseGeocode/\(userLocation.coordinate.latitude),\(userLocation.coordinate.longitude).json?key=FBSjYeqToGYAeG2A5txodKfGHrql38S4&radius=100") else { return }
                        ...
                        let task = URLSession.shared.dataTask(with: url) {(data, response, error) in
                            ...
                            do {
                                ...
                                let mapSnapshot = MapSnapshot(userLocation: userLocation, image: mapImage, addresses: newAddresses)
